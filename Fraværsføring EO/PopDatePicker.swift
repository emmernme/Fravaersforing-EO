...
public class PopDatePicker : NSObject, UIPopoverPresentationControllerDelegate, DataPickerViewControllerDelegate {
    ...
    public typealias PopDatePickerCallback = (newDate : NSDate, forTextField : UIView)->()
    ...
    public init(forTextField: UIView) {
    ...
    public func pick(inViewController : UIViewController, initDate : NSDate?, dataChanged : PopDatePickerCallback) {
        ...
        if let _popover = popover {
            ...
            _popover.sourceRect = CGRectMake(self.offset,inViewController.view.bounds.size.height,0,0)
            _popover.delegate = self
            self.dataChanged = dataChanged
			NSLog("%@", datePickerVC.description)
